...
// {
//     string md5 = inv::INV_MD5::md5bin(argv[1]);
//     printf("%u\n", *((uint32_t*)(md5.c_str()))%6);
//     return 0;
// }
// 
// #include <cassert>
...
//         }
//         gettimeofday(&e, NULL);
//         acc += (e.tv_sec - b.tv_sec) * 1000000 + (e.tv_usec - b.tv_usec);
//     }
//     printf("iterate: %f\n", acc*1.0/1000);
// 
//     acc = 0;
...
TEST(json, sax)
{
    ...
    uint32_t decayPeriod = 86400;
    rapidjson::StringStream ss(json.c_str());
    JsonSaxHandler jsh(decayRate, decayPeriod);
    rapidjson::Reader reader;

    ASSERT_EQ(true, reader.Parse<rapidjson::kParseNumbersAsStringsFlag>(ss, jsh));
    std::map<std::string, double> dom = DomParse(json, decayRate, decayPeriod);
    ASSERT_EQ(false, dom.empty());


    EXPECT_EQ(true, dom.size() == jsh.GetResult().size());
    for (const auto& it: jsh.GetResult())
    {
        EXPECT_DOUBLE_EQ(dom[it.first], it.second);
    }
}
