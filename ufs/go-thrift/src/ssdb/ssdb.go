...
func (c *Client) HashSet(key string, field string, val string) (interface{}, error) {
...
func (c *Client) HashGet(key string, field string) (interface{}, error) {
...
func (c *Client) recv() ([]string, error) {
	...
	for {
		resp := c.parse()
		if resp == nil {
            return nil, fmt.Errorf("nil resp1")
        }
        if len(resp) > 0 {
			return resp, nil
		}
