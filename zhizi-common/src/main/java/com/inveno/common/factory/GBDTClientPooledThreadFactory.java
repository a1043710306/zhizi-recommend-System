...
public class GBDTClientPooledThreadFactory extends BasePooledObjectFactory<TServiceClient>{

	private final static Logger logger = LoggerFactory.getLogger(GBDTClientPooledThreadFactory.class);
	...
	public TServiceClient create() throws Exception {
		...
		transport.open();
		return new PredictService.Client(protocol);
	}
