...
public class ElasticSearchHelper {
	...
	private static Log logger = LogFactory.getLog(ElasticSearchHelper.class);

	private static Map<String, ElasticSearchHelper> mInstance = new HashMap<String, ElasticSearchHelper>();

	private static ElasticSearchHelper instance = null;

	private Client client;

	public static synchronized ElasticSearchHelper newInstance(String[] arrHostPort, String clusterName) {
		return newInstance("default", arrHostPort, clusterName);
	}

	public static synchronized ElasticSearchHelper newInstance(String alias, String[] arrHostPort, String clusterName) {
		ElasticSearchHelper instance = mInstance.get(alias);
		if (instance == null) {
			instance = new ElasticSearchHelper(arrHostPort, clusterName);
			mInstance.put(alias, instance);
		}
		return instance;
	}
	...
	//单列模式
	private ElasticSearchHelper(String[] arrHostPort, String clusterName) {
		logger.info("init ElasticSearchHelper");
		try {
			Settings settings = Settings.builder()
					.put("cluster.name", clusterName)
					.put("client.transport.sniff", true)
					.put("thread_pool.bulk.size", 8)
					.put("thread_pool.bulk.queue_size", 1000)
					.put("thread_pool.index.size", 8)
			...
			for (String hostport : arrHostPort) {
	...
	public BulkResponse bulkUpdate(Map<String, Map<String, Object>> mUpdateDoc, String index, String type) {
		BulkRequestBuilder bulkRequest  = client.prepareBulk();
