...
@Component("primarySelectionImpl")
public class PrimarySelectionImpl  extends PolicyAbstract<List<ResponParam>> {
	...
	public List<ResponParam> process(Context context) throws TException {
		...
		try {
			if (logger.isTraceEnabled()) {
				logger.trace(strRequestInfo + ", begin invoke PrimarySelectionImpl context " + JSON.toJSONString(context));
			}

			logger.info(strRequestInfo + ", begin invoke PrimarySelectionImpl");
			monitorLog.addCntLogByProduct(context, MonitorType.GMP_INVOKE_COUNT);
			monitorLog.addCntLogByProductAndScenario(context, MonitorType.GMP_INVOKE_COUNT);
			recommendInfoData = dubboPrimarySelection.process(context);

			context.setRecommendInfoData(recommendInfoData);
		} catch (Exception e) {
			monitorLog.addCntLogByProduct(context, MonitorType.GMP_FAIL_COUNT);
			monitorLog.addCntLogByProductAndScenario(context, MonitorType.GMP_FAIL_COUNT);
			logger.info(strRequestInfo + ", invoke PrimarySelectionImpl failed", e);
			...
			if (!bInvokedFailed && nDataCount <= 0) {
				...
				monitorLog.addCntLogByProductAndScenario(context, MonitorType.GMP_EMPTY_LIST_COUNT);
			}
			logger.info(strRequestInfo + ", end invoke PrimarySelectionImpl, info list size is " + nDataCount + " spend time:" + tsSpent + " ms ");
		}
