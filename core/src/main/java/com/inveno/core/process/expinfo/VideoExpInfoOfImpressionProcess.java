...
@Component("videoExpInfoOfImpressionProcess")
public class VideoExpInfoOfImpressionProcess extends AbstractExpInfoOfImpression {

	public static  Log logger = LogFactory.getLog(VideoExpInfoOfImpressionProcess.class);
	...
	public void getExpInfo(Context context, List<Double> indexListForNoCache, List<ResponParam> resultList, List<Double> indexList, Set<ResponParam> resultSet,boolean useImpression) {
		...
		Map<String, String> mExpInfoKey = getExpInfoKey(app, context.getLanguage(), true, false);
		...
		logger.debug("uid is :"+ context.getUid() + " mExpInfoKey is " + mExpInfoKey);
 		...
 		// 探索下发最大资讯数
		int expRange = (int)configMap.get("expRangeOfVideo");

		// 流量探索比例 配置0.15
		double flowExpPer = (double) configMap.get("flowExpPer2Video");
