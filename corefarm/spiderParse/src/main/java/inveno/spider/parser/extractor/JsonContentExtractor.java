...
public class JsonContentExtractor implements Extractor {
	...
	public JsonContentExtractor(Profile profile, ParseStrategy parseStrategy,
	...
	public List<Page> extract(Page page) {
		...
		try {
			...
			if (nextpageLink != null) {
				...
				Page nextPage = new Page(page.getId(), Utils.calculateLink(page
						.getBaseUrl(), nextpageLink, page.getCharset()),
						Extractor.Type.Content, page.getPageNum() + 1, page
								.getAncestorUrl());
			...
			if (summary == null)
			    summary = (String) page.getMeta(Page.Meta.summary);
			if (author == null)
			...
			if (date == null) {
				if (null == profile.getListingConfig().getDatePath()
						&& null == profile.getContentConfig().getDatePath()) {// 如没配置时间，取当天
			...
			Article article = new Article(url, date, title,summary, content, author,
					section, pageno, refererUrl);
    ...
    private String extractContent(Page page, int pageNum)
		...
		if(null!=profile.getListingConfig()?profile.getListingConfig().getIsContentPage():false){
	...
	private String extractSummary(Page page) throws ExtractException {
	    ...
	    if (summaryPath != null) {
	        String[] summarys = mParseStrategy.extractSummaryPath(page.getHtml(),
	                mContentConfig.getHtml2xml(), summaryPath);
	...
	private String extractAuthor(Page page) {
		try {
			...
			if (authorPath != null) {
				String[] authors = mParseStrategy.extractAuthorPath(page
						.getHtml(), mContentConfig.getHtml2xml(), authorPath);
	...
	private String extractSection(Page page) {
		try {
			...
			if (sectionPath != null) {
				String[] section = mParseStrategy.extractSectionPath(page
						.getHtml(), mContentConfig.getHtml2xml(), sectionPath);
	...
	private String extractPageno(Page page) {
		try {
			...
			if (pagenoPath != null) {
				String[] pageno = mParseStrategy.extractPagenoPath(page
						.getHtml(), mContentConfig.getHtml2xml(), pagenoPath);
	...
	private String extractNextPageLink(Page page) throws ExtractException {
		...
		if (nextpagePath != null
			...
			String nextpageLink = mParseStrategy.extractNextPagePath(page
					.getHtml(), mContentConfig.getHtml2xml(),
	...
	private boolean isTheFinalPage(final Page page,final String nextPageLink)
	{
	    ...
	    String url = Utils.calculateLink(page
                .getBaseUrl(), nextPageLink, page.getCharset());
        //if current link to the same page with next or ancestor,then it means to current page is the final page.
	    if(nextPageLink!=null)
	    {
