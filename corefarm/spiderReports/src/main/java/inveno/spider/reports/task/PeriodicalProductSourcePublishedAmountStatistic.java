 ...
 */
public class PeriodicalProductSourcePublishedAmountStatistic extends AbstractStatistic
{
	private static final Logger log = Logger.getLogger(PeriodicalProductSourcePublishedAmountStatistic.class);
	...
	private String targetTableName = "t_article_publish_source";
	...
	public PeriodicalProductSourcePublishedAmountStatistic()
	{
	...
	private String[] permutateKey(String source ,String[] arrProductId, String[] arrLanguage)
	{
		...
		for (int i = 0; i < arrProductId.length; i++)
		{
			...
			{
				String keyInfo = TextUtil.getString(new String[]{arrProductId[i], arrLanguage[j],source}, String.valueOf(SEPARATER_SOURCE_TYPE));
				hsKey.add(keyInfo);
			}
		}
		return (String[])hsKey.toArray(new String[0]);
	}
	private String[] permutateKey(String source ,String[] arrProductId, String[] arrLanguage, String[] arrContentType, String[] arrBodyImagesCountHint)
	{
		arrProductId  = addOptionAll(arrProductId);
		arrLanguage   = addOptionAll(arrLanguage);
		...
		for (int i = 0; i < arrProductId.length; i++)
		{
			for (int j = 0; j < arrLanguage.length; j++)
			{
				for (int k = 0; k < arrContentType.length; k++)
				{
					...
					{
						String keyInfo = TextUtil.getString(new String[]{arrProductId[i], arrLanguage[j], arrContentType[k], arrBodyImagesCountHint[m],source}, String.valueOf(SEPARATER_SOURCE_TYPE));
						hsKey.add(keyInfo);
						keyInfo = TextUtil.getString(new String[]{arrProductId[i], arrLanguage[j], arrContentType[k], arrBodyImagesCountHint[m],"all"}, String.valueOf(SEPARATER_SOURCE_TYPE));
						hsKey.add(keyInfo);
					}
				}
			}
	...
	private HashMap<String, Integer> getProductPublishCount(Date startTime, Date endTime)
	{
		...
		for (int i = 0; alContent != null && i < alContent.size(); i++)
		{
			...
			String source = (String)mContent.get("source");
			...
			if (fApplyContentType)
			{
				...
				String[] arrKeyInfo    = permutateKey(source,arrProductId, arrLanguage, arrContentType, arrBodyImagesCountHint);
			...
			else
			{
				String[] arrProductId = enumFirmApp( (String)mContent.get("firm_app") );
				String[] arrLanguage  = new String[]{ language };
				String[] arrKeyInfo   = permutateKey(source,arrProductId, arrLanguage);
				for (int j = 0; j < arrKeyInfo.length; j++)
				{
					mProductPublishCount = increaseAmount(mProductPublishCount, arrKeyInfo[j], increament);
				}
	...
	public void reportStatistic(Date startTime, Date endTime) throws Exception
	{
		try
		{
			...
			{
				String[] primaryKeyFields = (fApplyContentType) ? new String[]{"timestamp_day", "timestamp_hour", "timestamp_min", "language", "product_id", "content_type", "body_images_count","source"}
																: new String[]{"timestamp_day", "timestamp_hour", "timestamp_min", "language", "product_id","source"};
	...
	private static void printCliHelp()
	{
		...
		formatter.printHelp("java " + PeriodicalProductSourcePublishedAmountStatistic.class.getCanonicalName(), createOptions());
	}
	...
	public static void main(String[] args)
	{
		...
		int nAlignedIncrement1 = 10;

		int aligned_minute1 = ((int)c1.get(Calendar.MINUTE) / nAlignedIncrement1 - 1) * nAlignedIncrement1;
		c1.set(Calendar.MINUTE, aligned_minute1);
		c1.set(Calendar.SECOND, 0);
		startTime1 = c1.getTime();
		c1.add(Calendar.MINUTE, nAlignedIncrement1);
	
		Date finishTime1 = c1.getTime();
		c1.setTime(startTime1);
		System.out.println(startTime1);
		System.err.println(finishTime1);
		...
		try
		{
			...
			PeriodicalProductSourcePublishedAmountStatistic task = new PeriodicalProductSourcePublishedAmountStatistic();
			...
			int timeLast = 24;
			if (cmd.hasOption("timeLast"))
			{
				timeLast =  Integer.parseInt(cmd.getOptionValue("timeLast"));
			}
			
			task.setAvailableLanguage( cmd.getOptionValue("availableLanguage") );

			int nAlignedIncrement = 1;
			int periodIncrementField = Calendar.HOUR_OF_DAY;

			Calendar c = Calendar.getInstance();
			Date startTime  = null;
			if (cmd.hasOption("date"))
			{
				Date evaluateDate = DateUtil.stringToDate(cmd.getOptionValue("date"), "yyyy-MM-dd-HH:mm:ss");
				...
				startTime  = c.getTime();
				c.add(Calendar.DAY_OF_YEAR, 1);*/
				c.add(Calendar.HOUR_OF_DAY, -1* timeLast); 
				startTime  = c.getTime();
				c.add(Calendar.HOUR_OF_DAY, timeLast); 	
			}
			else
			{
				...
				c.set(Calendar.MINUTE, 0);
				c.set(Calendar.SECOND, 0);
				c.add(Calendar.HOUR_OF_DAY, -1*nAlignedIncrement);
				startTime = c.getTime();
				c.add(Calendar.HOUR_OF_DAY, nAlignedIncrement);
